"""
V4.1 Camera-Range Evaluation Script -- run from THIS folder.

    cd PPOagent/saved_agents/V4.1_camera_fixed
    python evaluate_V4_1_camera.py --seeds 5

Relies on the local sumo_rl_env.py shim (re-exports sumo_rl_env_V4_1_camera.py)
so that evaluate_models.py's hardcoded `from sumo_rl_env import ...` resolves
to the camera-limited environment instead of src/sumo_rl_env.py.
...
def main():
    ...
    with open(os.path.join(eval_dir, "evaluation_config.txt"), "w") as f:
        f.write(f"Run: {timestamp}\nSeeds: {seeds}\nAgent: V4.1 camera-range (diff_waiting_time, CAMERA_RANGE=100m)\n")
    ...
    models_to_test = [
        {"name": "Maskable_PPO_V4.1", "type": "ppo",   "path": model_path},
