"""
V3 Architecture: Binary "Keep or Switch" Traffic Light Controller
================================================================
...
- Pressure-based reward with exponential starvation penalty (>90s threshold).
- Compact 13-feature observation: [phase_onehot(4), elapsed(1), lane_demands(8)]
"""
...
class SwitchOrKeepWrapper(gym.Wrapper):
    ...
    - Reward computation (pressure + exponential starvation penalty)
    ...
    def action_masks(self):
        ...
        - elapsed < MIN_GREEN  → [True, False]  (force Keep: safety minimum)
        - elapsed >= MAX_GREEN → [False, True]   (force Switch: prevent hogging)
        - otherwise            → [True, True]    (agent decides freely)
        """
        can_keep = self.elapsed_green_time < self.MAX_GREEN
        can_switch = self.elapsed_green_time >= self.MIN_GREEN

        # Safety net: at least one action must always be valid
    ...
    def _compute_reward(self):
        """
        Reward = Pressure / num_lanes - Starvation Penalty

        Pressure: (outgoing vehicles) - (incoming vehicles)
            Positive when the intersection is clearing traffic efficiently.

        Starvation Penalty: Smooth, non-linear penalty that begins at 30 seconds
            to give the agent a "warning track" before the penalty explodes.
            This prevents the Low Traffic Anomaly where the agent ignores a single
            stranded vehicle because the 90-second cliff provides zero gradient signal.

            Penalty values:
              30s → 0.05  |  60s → ~0.29  |  90s → ~1.70
              120s → ~9.9 |  150s+ → 30.0 (capped to prevent gradient explosion)

            formula: 0.05 * 1.5^((max_wait - 30) / 10),  capped at 30.0
        """
        ts = self._ts

        # Base reward: pressure
        incoming = sum(
            self._sumo.lane.getLastStepVehicleNumber(lane) for lane in ts.lanes
        )
        outgoing = sum(
            self._sumo.lane.getLastStepVehicleNumber(lane) for lane in ts.out_lanes
        )
        pressure = outgoing - incoming
        num_lanes = max(len(ts.lanes), 1)
        reward = pressure / num_lanes

        # Sum of Queues penalty (replaces the exponential starvation penalty)
        # This penalizes the agent for every vehicle standing still, preventing
        # the teleportation bug from rewarding the agent (as wait time is ignored).
        sum_of_queues = sum(
            self._sumo.lane.getLastStepHaltingNumber(lane) for lane in ts.lanes
        )
        
        # We subtract a small penalty for each stopped car (e.g. 0.1 per car)
        # to encourage the AI to clear queues, but without exploding gradients.
        queue_penalty = (sum_of_queues / num_lanes) * 0.1
