"""
V8 Architecture: V7 + hard empty-intersection mask on Switch action.

Changes from V7:

  1. action_masks(): if total_visible == 0, Switch is always masked out.
        V7 used a soft penalty (-0.03) to discourage switching on an empty
        intersection. That signal was too weak -- 4.66M vs Fixed_60s 4.61M
        gap remained unchanged. A hard mask is consistent with MIN_GREEN /
        MAX_GREEN philosophy: certain actions are structurally forbidden, not
        just discouraged. No gradient noise, no edge-case where the agent
        occasionally wastes a yellow transition in sparse traffic.

        Risk: a vehicle just outside the 150m camera range would be invisible.
        In low-traffic scenarios this is extremely rare and the gain from
        eliminating empty switches far outweighs the marginal cost.

  2. IDLE_SWITCH_PENALTY removed (mask makes it redundant).

Everything else is identical to V7 (150m camera, starvation@45s, diff_wait
reward, 21-dim obs, ghost-car logic).
"""
...
class SwitchOrKeepWrapper(gym.Wrapper):
    ...
    # --- V8 constants --------------------------------------------------------
    CAMERA_RANGE             = 150.0  # meters (same as V7)
    STARVATION_THRESHOLD     = 45.0   # seconds to starvation_score=1.0 (same as V7)
    STARVATION_PENALTY_COEF  = 0.05   # unchanged
    # IDLE_SWITCH_PENALTY removed -- replaced by hard mask in action_masks()
    ...
    def _compute_reward(self):
        """
        V8 reward = diff_waiting_time - starvation_penalty

        idle_switch_penalty removed -- the hard mask in action_masks() makes it
        structurally impossible to switch on an empty intersection, so the penalty
        is no longer needed.
        """
