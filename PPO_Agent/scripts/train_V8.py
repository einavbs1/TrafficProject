"""
V8 Training Script -- run from THIS folder.

    cd PPO_Agent/scripts
    python train_V8.py --timesteps 6000000

V8 changes over V7:
    - Hard empty-intersection mask: Switch is masked when total_visible == 0
    - Idle switch penalty removed (mask makes it redundant)
...
def main():
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        model = MaskablePPO(
            ...
            tensorboard_log=os.path.join(_ROOT, "tensorboard"))
        model.policy.optimizer.param_groups[0]["eps"] = 1e-5
    ...
    print(f"Training V8 for {args.timesteps} steps")
