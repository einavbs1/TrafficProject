# DQN Agent (superseded — archived here in Old_Versions)
...
`../../PhaseB/FlowGrid_Capstone_Project_Book.docx` (Section 2.3) for the
full, honest account of why, including the 34-run evaluation history showing
the DQN controller's instability. The current, submitted agent is
`../../PPO_Agent/`.

Pre-DQN, pre-RL prototype scripts (manual/rule-based signal simulators) live
under `legacy/`, right in this folder.
...
- `legacy/` — pre-DQN, pre-RL prototype scripts (manual/rule-based signal
  simulators: `4WaySimulator.py`, `phase1_traci.py`, `simpleSimulator.py`)
  that predate this agent entirely.
- `data/maps/` — saved intersection presets (each subfolder is one map +
  its trained policy, if any).
...
Despite living under `Old_Versions/`, this agent is fully self-contained and
still runs; only its place in the project's current lineup has changed.

**Desktop GUI:**
```
cd Old_Versions/DQN_Agent/gui
...
all of them):
```
cd Old_Versions/DQN_Agent/scripts
python run_menu.py
```
