"""
V6 Camera Evaluation Script -- run from THIS folder.

    cd PPOagent/saved_agents/V6
    python evaluate_V6_camera.py --seeds 5

Uses the local sumo_rl_env.py shim (re-exports sumo_rl_env_V6_camera.py) so
evaluate_models.py resolves to the camera-limited environment, not src/sumo_rl_env.py.
...
def print_extra_metrics(eval_dir, map_name, seeds):
    ...
    summary["Switch_Rate_per100s"] = (
        summary["Total_Switches"] / 200
    ).round(1)
...
def main():
    ...
    with open(os.path.join(eval_dir, "evaluation_config.txt"), "w") as f:
        ...
        f.write(f"Agent: V6-camera (diff_waiting_time + starvation_penalty + CAMERA_RANGE=100m)\n")
        f.write(f"Extra metrics: Wait_Per_Vehicle, Switch_Rate_per100s\n")

    print(f"V6-CAMERA EVALUATION  |  results -> {eval_dir}\n", flush=True)
    ...
    models_to_test = [
        {"name": "Maskable_PPO_V6_camera", "type": "ppo",   "path": model_path},
    ...
    for m in maps:
        ...
        print_extra_metrics(eval_dir, m["name"], seeds)

    print("\nV6-camera Evaluation complete.")
