"""
V6 Evaluation Script -- run from THIS folder.

    cd PPOagent/saved_agents/V6
    python evaluate_V6.py --seeds 5

Auto-finds latest model from ./models/
Saves results to ./results/

Extra metrics vs previous versions:
    Wait_Per_Vehicle  -- Total_Wait_Time / Total_Arrived (normalised for traffic volume)
...
def print_extra_metrics(eval_dir, map_name, seeds):
    """Read the raw CSV and print extra metrics after evaluate_scenario runs."""
    ...
    summary["Switch_Rate_per100s"] = (
        summary["Total_Switches"] / 200   # 20000s episode / 100
    ).round(1)
...
def main():
    ...
    with open(os.path.join(eval_dir, "evaluation_config.txt"), "w") as f:
        ...
        f.write(f"Agent: V6 (diff_waiting_time + starvation_penalty)\n")
        f.write(f"Extra metrics: Wait_Per_Vehicle, Switch_Rate_per100s\n")

    print(f"V6 EVALUATION  |  results -> {eval_dir}\n", flush=True)
    ...
    models_to_test = [
        {"name": "Maskable_PPO_V6", "type": "ppo",   "path": model_path},
