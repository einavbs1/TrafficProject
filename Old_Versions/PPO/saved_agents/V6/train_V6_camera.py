"""
V6 Camera Training Script -- run from THIS folder.

    cd PPOagent/saved_agents/V6
    python train_V6_camera.py --timesteps 6000000

Differences from train_V6.py:
    - Imports sumo_rl_env_V6_camera (camera range 100m + starvation penalty)
    - Resume branch uses lr=1e-5, ent_coef=0.02 (corrected from train_V6.py)
...
def main():
    ...
    if resume_path:
        ...
        print(f"Fresh V6-camera agent: {agent_id}")
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        # lr=1e-5 conservative -- the 3e-4->0 schedule already converged;
        # ent_coef=0.02 unchanged to keep exploration consistent with fresh training.
        ...
        print("Building fresh V6-camera agent...")
        model = MaskablePPO(
            "MlpPolicy", vec_env,
    ...
    print(f"Training V6-camera for {args.timesteps} steps")
