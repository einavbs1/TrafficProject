"""
V7 Evaluation Script -- run from THIS folder.
...
def main():
    ...
    with open(os.path.join(eval_dir, "evaluation_config.txt"), "w") as f:
        ...
        f.write(f"Agent: V7 (diff_wait + starvation@45s + idle_switch_penalty + camera150m)\n")

    print(f"V7 EVALUATION  |  results -> {eval_dir}\n", flush=True)
    ...
    models_to_test = [
        {"name": "Maskable_PPO_V7", "type": "ppo",   "path": model_path},
