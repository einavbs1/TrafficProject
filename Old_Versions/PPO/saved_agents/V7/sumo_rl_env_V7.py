"""
V7 Architecture: diff_waiting_time + starvation penalty + camera range (150m) + idle switch penalty.

Changes from V6-camera:

  1. CAMERA_RANGE: 100m -> 150m
        V6-camera cut off too early in dense traffic -- queues extend beyond 100m.
        150m gives better queue visibility in high traffic without going back to
        full-lane omniscience.

  2. STARVATION_THRESHOLD: 90s -> 45s
        V6-camera penalty fired too late for sparse-traffic learning.
        At 45s threshold a vehicle waiting 15s already scores 0.33 (was 0.17).
        Signal is twice as strong at any given wait time, giving the policy a
        clearer gradient to serve isolated vehicles faster.

  3. Idle switch penalty: -0.03 when agent switches on empty intersection
        When the camera sees zero vehicles across all lane groups, switching is
        pure waste -- 3 seconds of yellow dead time with zero benefit. The agent
        previously had no reason not to switch since reward ~ 0 either way.
        Penalty cached from _compute_observation() (no extra TraCI calls).

Everything else is identical to V6-camera (same obs shape 21-dim, same reward
structure, same ghost-car logic, same action masking).
"""
...
class SwitchOrKeepWrapper(gym.Wrapper):
    ...
    # --- V7 constants --------------------------------------------------------
    CAMERA_RANGE             = 150.0  # meters (was 100m in V6-camera)
    STARVATION_THRESHOLD     = 45.0   # seconds to starvation_score=1.0 (was 90s)
    STARVATION_PENALTY_COEF  = 0.05   # unchanged
    IDLE_SWITCH_PENALTY      = 0.03   # penalty for switching on empty intersection
    ...
    def __init__(self, env):
        ...
        self._total_visible = 0      # total visible vehicles across all groups (cached)
        self._last_action = 0        # cached action for idle-switch penalty
    ...
    def step(self, action):
        ...
        self._last_action = action  # cache before simulation steps
        ...
        reward = self._compute_reward()    # reads _last_action, _max_starvation, _total_visible
    ...
    def _compute_observation(self):
        ...
        Also caches self._max_starvation and self._total_visible for _compute_reward().
        """
        ...
        self._total_visible = total_visible  # cached for idle-switch penalty

        return np.array(phase_onehot + [elapsed_norm] + lane_demands + lane_starvation,
                        dtype=np.float32)

    def _compute_reward(self):
        """
        V7 reward = diff_waiting_time - starvation_penalty - idle_switch_penalty

        diff_waiting_time : full ground truth (all lanes, all vehicles)
        starvation_penalty: max(starvation_scores) * 0.05
                            starvation_score = wait / STARVATION_THRESHOLD (45s)
        idle_switch_penalty: IDLE_SWITCH_PENALTY (0.03) when agent switches
                            and camera sees zero vehicles in all lane groups.
                            Prevents pointless 3s yellow transitions in an
                            empty intersection. No extra TraCI calls -- reuses
                            self._total_visible and self._last_action cached
                            in _compute_observation() and step().
        """
        ...
        # starvation penalty (camera-limited, fires at 45s now)
        starvation_penalty = self._max_starvation * self.STARVATION_PENALTY_COEF

        # idle switch penalty (zero extra TraCI calls)
        idle_penalty = 0.0
        if self._last_action == 1 and self._total_visible == 0:
            idle_penalty = self.IDLE_SWITCH_PENALTY

        return float(diff_wait - starvation_penalty - idle_penalty)
