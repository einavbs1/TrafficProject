"""
V4.1 Camera-Range Training Script -- run from THIS folder.

    cd PPOagent/saved_agents/V4.1_camera_fixed
    python train_V4_1_camera.py --timesteps 6000000

Difference from V4_initial/train_V4_initial.py:
    Uses sumo_rl_env_V4_1_camera.py, which limits demand/starvation observation
    to vehicles within CAMERA_RANGE=100m of the stop line (see that file's
    docstring). Fresh training only -- the 21-dim obs space is the same shape
    as V4 but the values have different meaning, so V4 weights are not reused.
